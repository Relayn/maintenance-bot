...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if db_user.name != current_name:
        logger.info(f"Updating name for user {user.id}: '{db_user.name}' -> '{current_name}'")
    ...
    logger.info(f"Authorized user {user.id} ({db_user.name}) with role '{db_user.role}' started the bot.")
...
async def unauthorized_user_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    ...
    logger.warning(f"Received message from unauthorized user {user.id} ({user.first_name}).")
